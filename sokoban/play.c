...
extern short rows, columns, level, moves, pushes, savepack, packets;
...
short play() {
   ...
   while( ret == 0) {
	 ...
	 case 'q':    /* quit the game 					*/
	  	      move( MAXROW+2,0);
		      printw("Quit and save game?");
		      if (toupper(quit_flag = getch())=='N')
	                ret = E_ENDGAME; 
	  	      else if (toupper(quit_flag)=='Y')
			     if( (ret = savegame()) == 0)
		               ret = E_SAVED;
		      move( MAXROW+2,0);
		      deleteln();
	              break;
         case 26 :    ret = E_ENDGAME;
                      break;
         case 'b':    /* boost priority -- super slow on VMS            */
 		      sys$setpri(0,0,4,0);
                      break;
	 ...
	 case 'i':    /* up 						*/
	 case 'I':    /* run up 					*/
	 case CNTL_I: /* run up, stop before object 			*/
	 case 'k':    /* down 						*/
	 case 'K':    /* run down 					*/
	 case CNTL_K: /* run down, stop before object 			*/
	 case 'l':    /* right 						*/
	 case 'L':    /* run right 					*/
	 case CNTL_L: /* run right, stop before object 			*/
	 case 'j':    /* left 						*/
	 case 'J':    /* run left 					*/
	 case CNTL_J: /* run left, stop before object 			*/
		      do {
...
register short action;
{
   ...
   if( (action == 'i') || (action == 'k')) {
      tpos1.x = (action == 'i') ? ppos.x-1 : ppos.x+1;
      tpos2.x = (action == 'i') ? ppos.x-2 : ppos.x+2;
      tpos1.y = tpos2.y = ppos.y;
   }
   else {
      tpos1.y = (action == 'j') ? ppos.y-1 : ppos.y+1;
      tpos2.y = (action == 'j') ? ppos.y-2 : ppos.y+2;
